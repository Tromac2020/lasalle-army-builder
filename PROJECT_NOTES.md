...
### Round 9: print race condition — icons/resolve bursts silently missing on real-world prints

Troy pushed round 8 live, then printed real cards from the deployed site (`v1.0.1`, then again confirmed still broken at `v1.0.2`) and found trait icons and resolve-burst icons missing across the board — not just on artillery, on every card type (infantry, cavalry, artillery), for whichever nation he tried. He supplied his own print-to-PDF output as evidence, plus a reference screenshot of correctly-rendered cards for comparison. Critically, round 8's own local verification (Playwright, `page.emulateMedia`) had never caught this, because it doesn't call `window.print()` for real — it only fakes print CSS on an already-loaded page.

Root cause, found in `App.tsx`: the print flow was `setTimeout(() => window.print(), 50)` — a flat 50ms delay between switching into print mode and opening the print dialog, on the assumption that would be enough for React to render the print view. It isn't enough for the *images inside* that view to finish loading: card icon PNGs (`src/assets/cards/*.png`) range from a few KB up to ~60KB (the resolve-burst images are the biggest), aren't yet cached on someone's first visit, and Chrome's print rendering doesn't wait for a still-loading `<img>` — it just leaves that spot blank, with no error, no broken-image icon, nothing. Which images "won by the 50ms mark" was effectively random based on network timing, which is exactly the flaky, everything-except-specific-things-missing symptom Troy kept reporting across multiple rounds.

Fix: replaced the fixed timeout with an actual wait for every `<img>` inside the print view (now `id="print-root"`) to finish loading (or fail) before calling `window.print()`, capped at a 4s fallback so printing can never hang forever if an image genuinely never loads. Verified by intercepting network requests in Playwright to add a realistic 300-900ms artificial delay to every card PNG (simulating a real first-time page load) and confirming `window.print()` (mocked) now only fires once every image in `#print-root` reports `complete`/`naturalWidth > 0` — previously this would have fired at 50ms with most images still in flight. Screenshotted the result under the same throttled conditions: every trait icon and resolve burst rendered correctly. Version bumped to `1.0.3`.
