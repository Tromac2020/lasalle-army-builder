...
Icon artwork for skirmish badges, resolve bursts, and all 11 trait icons (`src/components/icons.tsx`) is original SVG — simple geometric shapes conveying the same info as the booklet's icons (a flag for Resilient, crossed blades for Attack Columns, a wheel for Horse Artillery, etc.) rather than any reproduction/trace of the source PDF's artwork, per copyright constraints on this tool.
